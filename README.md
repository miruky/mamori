...
難易度はやさしい・ふつう・むずかしいから選べる。難しいほど敵が硬く賞金が減り、残機も少ない。難易度ごとに最高到達ウェーブと勝利数を記録し、HUDの「最高」に現在の難易度の最高記録を出す。難易度・テーマ・戦績はブラウザに保存され、次回も引き継がれる。
...
- `src/lib/game.ts` 設置・ウェーブ進行・照準・命中・経済・勝敗を持つ中核
...
- 進行中の盤面はリロードで失われる。難易度・テーマ・戦績は保存するが、配置した塔やウェーブの途中経過は復元しない。
